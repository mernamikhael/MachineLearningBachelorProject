...
def user(userID,per):
    ...
    with open(pathA, 'rb') as files:
        ...
        for anomaly in postsA:
            # check date time
            datetime_object = datetime.strptime(anomaly[3][:-4],"%Y-%m-%dT%H:%M:%S+")    
            hour=datetime_object.hour
            month=datetime_object.month
            year=datetime_object.year
            day=datetime_object.day
    ...
    scoreB1=2
    import topicModel
    scoreTA,scoreTB= topicModel.topic(userID,userID,scoreA1,scoreB1,1)


    import lang
    scoreLA,scoreLB= lang.language(langu, languA,total_posts_per_year,total_posts_per_yearA,scoreA1,scoreB1,1)
    # print added,updated,posted,shared
    # print addedA,updatedA,postedA,sharedA


    import typeOfPost
    scorePA,scorePB= typeOfPost.type_post(added,shared,updated,posted,total_posts_per_year,addedA,sharedA,updatedA,postedA,total_posts_per_yearA,personalityTypePost[per],scoreA1,scoreB1,1)

    
    import tagged

    scoreTagA,scoreTagB= tagged.tagged(tags,tagnbr,taggedposts,total_posts_per_year,tagsA,tagnbrA,taggedpostsA,total_posts_per_yearA,scoreA1,scoreB1,1)
    ...
    weekposts51A,weekposts50A,personalityDay[per],scoreA1,scoreB1,1)
